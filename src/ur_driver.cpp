...
UrDriver::UrDriver(std::condition_variable & rt_msg_cond,
                   ...
                   double servoj_gain)
: maximum_time_step_(max_time_step), minimum_payload_(min_payload), maximum_payload_(max_payload),
  REVERSE_PORT_(reverse_port), servoj_time_(servoj_time), servoj_lookahead_time_(servoj_lookahead_time),
...
std::array<double, 6> UrDriver::interp_cubic(double t,
                                             double T,
                                             ...
                                             const std::array<double, 6> & p1_vel)
{
  ...
  for(unsigned int i = 0; i < p0_pos.size(); i++)
  {
    double a = p0_pos[i];
    double b = p0_vel[i];
    double c = (-3 * p0_pos[i] + 3 * p1_pos[i] - 2 * T * p0_vel[i] - T * p1_vel[i]) / std::pow(T, 2);
    double d = (2 * p0_pos[i] - 2 * p1_pos[i] + T * p0_vel[i] + T * p1_vel[i]) / std::pow(T, 3);
    positions[i] = a + b * t + c * std::pow(t, 2) + d * std::pow(t, 3);
  }
  return positions;
}
...
void UrDriver::servoj(const std::array<double, 6> & positions, int keepalive)
{
...
void UrDriver::servoj(const double * positions, int keepalive)
{
  servoj(*reinterpret_cast<const std::array<double, 6> *>(positions));
}
...
void UrDriver::closeServo(const std::array<double, 6> & positions)
{
...
void UrDriver::setSpeed(const std::array<double, 6> & joint_speed, double acc)
{
  rt_interface_->setSpeed(joint_speed, acc);
}

void UrDriver::setSpeed(const double * joint_speed, double acc)
{
  rt_interface_->setSpeed(joint_speed[0], joint_speed[1], joint_speed[2], joint_speed[3], joint_speed[4],
                          joint_speed[5], acc);
}

const std::array<std::string, 6> & UrDriver::getJointNames()
{
  return joint_names_;
}

void UrDriver::setJointNames(const std::array<std::string, 6> & jn)
{
  joint_names_ = jn;
}
