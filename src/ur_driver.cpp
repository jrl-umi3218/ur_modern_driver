...
std::array<double, 6>
UrDriver::interp_cubic(double t, double T, const std::array<double, 6>& p0_pos,
...
bool UrDriver::doTraj(
    const std::vector<double>& inp_timestamps,
...
bool UrDriver::openServo() {
    ...
    new_sockfd_ =
        accept(incoming_sockfd_, (struct sockaddr*)&cli_addr, &clilen);
    if (new_sockfd_ < 0) {
...
bool UrDriver::start() {
    ...
    print_debug("Listening on " + ip_addr_ + ":" +
                std::to_string(REVERSE_PORT_) + "\n");
    return true;
}
...
void UrDriver::setSpeed(const double* joint_speed, double acc) {
    rt_interface_->setSpeed(joint_speed[0], joint_speed[1], joint_speed[2],
                            joint_speed[3], joint_speed[4], joint_speed[5],
                            acc);
}
